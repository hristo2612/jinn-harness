...
| Provider | `jinn-engine-echo` | The no-CLI provider: answers from the prompt itself. The EXTENSION proof (a third engine added by a profile edit alone) and the seam's CI-runnable evidence, since it needs no vendor authentication. |
